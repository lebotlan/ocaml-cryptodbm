...
let append_subtable table sconf =
  let subtable =
    ...
    | None -> Cryptodbm.append_uncrypted_subtable table ~name:sconf.name ~signwd:sconf.signed ~check_signature:(sconf.signed <> "") ()
    | Some passwd -> Cryptodbm.append_subtable table ~name:sconf.name ~passwd ~signwd:sconf.signed ~check_signature:(sconf.signed <> "") ()
  in
  (sconf, subtable)
...
let execute state tconf = function
  | Create sconf ->
	...
	match sconf.crypted with
	| None ->
	    Cryptodbm.create_uncrypted_subtable state.table ~name:sconf.name ~signwd:sconf.signed ()
	| Some passwd ->
	    Cryptodbm.create_subtable state.table ~name:sconf.name ~passwd ~signwd:sconf.signed 
  ...
  | Reopen ->
      ...
      let table = 
	Cryptodbm.open_append ~file:(Cryptodbm.get_rootfile state.table) ~passwd:tconf.passwd ~signwd:tconf.signwd ~check_signature:(tconf.signwd <> "")
      in
      state.table <- table ;
...
let check_subconformity table state sconf =
  ...
  let subtable = 
    match sconf.crypted with
    | None -> Cryptodbm.open_uncrypted_subtable table ~name:sconf.name ~signwd:sconf.signed ()
    | Some passwd -> Cryptodbm.open_subtable table ~name:sconf.name ~passwd ~signwd:sconf.signed ()
  in
...
let check_conformity ~only_uncrypted tconf state =
  ...
  let table =
    if only_uncrypted then Cryptodbm.open_only_uncrypted ~file:tconf.filename ~signwd:tconf.signwd
    else Cryptodbm.open_read ~file:tconf.filename ~passwd:tconf.passwd ~signwd:tconf.signwd
  in
...
(*  Printf.printf "Sequence = %s\n%!" (sequence2s sequence) ; *)
  ...
  let table = Cryptodbm.open_create ~file:tconf.filename ~overwrite:true ~passwd:tconf.passwd ~signwd:tconf.signwd
